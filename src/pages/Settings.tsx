...
const APIKeyInput = ({ provider, apiKey, setApiKey, testKey, saveKey, validating }) => (
  <div className="space-y-2">
    <Label htmlFor={provider}>{`${provider.charAt(0).toUpperCase() + provider.slice(1)} API Key`}</Label>
    <Input
      id={provider}
      type="text"
      value={apiKey}
    ...
    />
    <div className="flex gap-2 pt-2">
      <Button variant="outline" onClick={testKey} disabled={validating || !apiKey.trim()}>
        {validating ? 'Testing...' : 'Test Key'}
      </Button>
      <Button onClick={() => saveKey(provider)}>Save</Button>
    </div>
  </div>
);
