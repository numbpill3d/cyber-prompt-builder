...
export function createMemoryService(type: string, config?: MemoryProviderConfig): MemoryService {
  switch (type.toLowerCase()) {
    case 'chroma':
      // Dynamic import to avoid circular dependencies
      const ChromaProvider = import('./chroma-memory-provider').then(module => {
        return new module.ChromaMemoryProvider(config);
      });
      return ChromaProvider as unknown as MemoryService;
    default:
...
export function getMemoryService(config?: MemoryProviderConfig): MemoryService {
  if (!defaultMemoryService) {
    defaultMemoryService = createMemoryService('chroma', config);
  }
