...
Please format your response exactly as follows (as valid JSON):
{
  ...
  "complexity": "${complexity === 'auto' ? 'medium' : complexity}",
...
The JSON must be valid. Each step should be focused on one aspect of the task. The 'prompt' field for each step should be detailed enough for an AI to generate the correct code.`
    ...
    try {
      ...
      return {
        ...
        steps,
        complexity: planData.complexity || complexity,
      ...
      return this.createSimpleFallbackPlan(prompt, complexity as 'simple' | 'medium' | 'complex');
    }
  }
   ...
   */
  private createSimpleFallbackPlan(prompt: string, complexity: 'simple' | 'medium' | 'complex'): TaskPlan {
    const planId = this.generateId('plan');
    ...
    if (complexity === 'simple') {
      ...
      steps = [
        ...
        },
        {
          ...
          dependencies: [steps?.[0]?.id, steps?.[1]?.id, steps?.[2]?.id],
          optional: true,
          status: 'pending',
