...
export function createUserPreferences(content?: string, preset?: keyof typeof USER_PREFERENCE_PRESETS): string {
  ...
  if (preset && USER_PREFERENCE_PRESETS[preset]) {
    ...
    if (layer && 'setPreferences' in layer) {
      // Type cast to the proper layer type
      (layer as import('./layers/user-preferences-layer').UserPreferencesLayer)
        .setPreferences(USER_PREFERENCE_PRESETS[preset]);
    }
  }
...
export function createCompletePrompt(options: {
  ...
  if (options.userPreferences) {
    ...
    if (layer && 'setPreferences' in layer) {
      // Type cast to the proper layer type
      (layer as import('./layers/user-preferences-layer').UserPreferencesLayer)
        .setPreferences(options.userPreferences);
    }
  }
  ...
  if (options.memoryEntries && options.memoryEntries.length > 0) {
    ...
    if (layer && 'addEntry' in layer) {
      for (const entry of options.memoryEntries) {
        // Type cast to the proper layer type
        (layer as import('./layers/memory-layer').MemoryLayer).addEntry({
          ...entry,
          timestamp: new Date()
        });
      }
...
export function examplePromptComposition() {
  ...
  if (taskLayer && 'addExample' in taskLayer) {
    ...
    typedTaskLayer.addExample('Input: "hello world" → Output: "helloWorld"');
    typedTaskLayer.addExample('Input: "user-profile-data" → Output: "userProfileData"');
  }
  ...
  if (memoryLayer && 'addEntry' in memoryLayer) {
    // Type cast to the proper layer type
    (memoryLayer as import('./layers/memory-layer').MemoryLayer).addEntry({
  ...
  if (prefsLayer && 'setPreferences' in prefsLayer) {
    // Type cast to the proper layer type
    (prefsLayer as import('./layers/user-preferences-layer').UserPreferencesLayer).setPreferences({
...
// Export constants and enums for easier access
export { DEFAULT_SYSTEM_PROMPTS, TASK_INSTRUCTION_TEMPLATES, USER_PREFERENCE_PRESETS };
export { MemoryEntryType, ResponseTone, ResponseFormat };